class SocketEdgeTTS {
    constructor(_indexpart, _filename, _filenum,
        ...
        this.custom_save_path = "/storage/emulated/0/Music/1";
    ...
    async saveFiles(blob) {
        if (this.start_save == false) {
            ...
            try {
                const rootHandle = await navigator.storage.getDirectory(); // For modern browsers
                const musicFolderHandle = await rootHandle.getDirectoryHandle("Music", { create: true });
                const customFolderHandle = await musicFolderHandle.getDirectoryHandle("1", { create: true });

                const fileHandle = await customFolderHandle.getFileHandle(this.my_filename + " " + this.my_filenum + '.mp3', { create: true });
